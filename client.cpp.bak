 ...
 * packet. In other cases, no message is received. 
 */
int open_the_door(int sockfd, NAT_type local_NAT, NAT_type peer_NAT) {
    ...
    if (peer_NAT == NAT_symmetric) {
        if (local_NAT == NAT_port_restricted) {
            cout << "Peer NAT type is symmetric, local NAT type is port restricted\n"
        ...
        } else if (local_NAT == NAT_restricted) {
            // Open the door
            cout << "Peer NAT type is symmetric, local NAT type is restricted\n"
                 << "Open the door to peer " << addr2str(peer_addr) << endl;
            sendto(sockfd, &pack, sizeof(pack), 0, (sockaddr *)&peer_addr, sizeof(peer_addr));
        }
        // Wait for peer's response to detect the actual peer port number
        ...
        if (local_NAT == NAT_symmetric) {
            ...
            } else if (peer_NAT == NAT_restricted) {
